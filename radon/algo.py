...
def sample_smc_blackjaxpretuning(
    ...
    sampler = build_smc_with_kernel2(prior_log_prob=get_jaxified_logprior(model),
                                     loglikelihood=get_jaxified_loglikelihood(model),
                                     target_ess=target_essn,
                                     num_mcmc_steps=num_mcmc_steps,
                                     n_particles=n_particles,
                                     dimentions=posterior_dimensions
                                     )
...
class SMCDiagnostics(NamedTuple):
    ...
    def update_diagnosis(i, history, info, state):
        ...
        return SMCDiagnostics(
            le.at[i].set(state.sampler_state.lmbda),
...
def add_to_inference_data(
    ...
    experiment_parameters = {
        ...
        "num_mcmc_steps": num_mcmc_steps,
        "iterations": np.array(total_iterations),
...
def more_than_pretuning(
    ...
    sampler = build_smc_with_kernel3(prior_log_prob=get_jaxified_logprior(model),
                                     loglikelihood=get_jaxified_loglikelihood(model),
                                     target_ess=target_essn,
                                     num_mcmc_steps=num_mcmc_steps,
                                     n_particles=n_particles,
